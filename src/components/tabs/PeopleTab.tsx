import { Mail, Phone, Flag, Briefcase, Plus } from "lucide-react";
      ...
      <PitchbookGate>
        <div>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {pb.leadership.map((m) => {
              const notInThrive = m.inThrive === false;
              return (
                <div
                  key={m.id}
                ...
                >
                  ...
                  {notInThrive ? (
                    <button
                      className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-brand-200 bg-brand-50 px-2 py-1 text-[12px] font-medium text-brand-700 transition-colors hover:bg-brand-100"
                      title="Not in Thrive — add this person"
                    >
                      <Plus className="h-3 w-3" /> Add to Thrive
                    </button>
                  ) : (
